...
function tokenize(expr) {
    ...
    while (i < expr.length) {
        ...
        if (/[a-zA-Z_]/.test(expr[i])) {
            ...
            if (s === 'and' || s === 'or' || s === 'not') {
                tokens.push({ type: 'keyword', value: s });
            } else {
...
function parseExpr(tokens) {
    ...
    function parseOr() { // наименьший приоритет: or
        let left = parseAnd();
    ...
    function parseVariable() { // 5 уровень приоритета наивысший (числа, переменные, скобки)
        ...
        if (t.type === 'ident') {
            ...
            if (peek()?.type === 'lparen') {
                ...
                if (peek()?.type !== 'rparen') {
                    args.push(parseOr());
                    while (peek()?.type === 'comma') { consume(); args.push(parseOr()); }
                }
                expect(')');
...
function buildAST(blocks, startIndex) {
    ...
    while (i < blocks.length) {
        ...
        if (kind === 'Set') {
            const varName = getField(el, 'varName');
            const rawVal = getField(el, 'value') || '0';
...
function run(instructions) {
    ...
    while (ip < instructions.length) {
        ...
        switch (instr.type) {
            ...
            case 'and': b = stack.pop(); a = stack.pop(); stack.push(Boolean(a) && Boolean(b)); break;
            case 'or':  b = stack.pop(); a = stack.pop(); stack.push(Boolean(a) || Boolean(b)); break;
            case 'not': { const v = stack.pop(); stack.push(!Boolean(v)); break; }
...
function validateProgramStructure(blockList) {
    const lastBlock = blockList[blockList.length - 1];
    if (!lastBlock || blockKind(lastBlock) !== 'End') {
        return { valid: false, error: 'Программа должна заканчиваться блоком End' };
    }

    let i = 0;
    while (i < blockList.length) {
        const kind = blockKind(blockList[i]);

        if (kind === 'If') {
            let hasCondEnd = false;
            let hasElse = false;
            let depth = 1;
            let j = i + 1;

            while (j < blockList.length && depth > 0) {
                const nextKind = blockKind(blockList[j]);
                
                if (nextKind === 'If') {
                    depth++;
                } else if (nextKind === 'Condition End') {
                    depth--;
                    if (depth === 0) {
                        hasCondEnd = true;
                    }
                } else if (nextKind === 'Else' && depth === 1) {
                    hasElse = true;
                }
                j++;
            }
            ...
            if (!hasCondEnd) {
                return { valid: false, error: 'После блока If должен быть блок Condition End' };
            }
        ...
        if (kind === 'Else') {
            ...
            if (!hasCondEnd) {
                return { valid: false, error: 'После блока Else должен быть блок Condition End' };
            }
        ...
        if (kind === 'While') {
            ...
            while (j < blockList.length && depth > 0) {
                ...
                if (nextKind === 'While' || nextKind === 'For') {
                    depth++;
                } else if (nextKind === 'Cycle End') {
                    depth--;
                    if (depth === 0) {
                        hasCycleEnd = true;
                    }
            ...
            if (!hasCycleEnd) {
                return { valid: false, error: 'После блока While должен быть блок Cycle End' };
            }
        ...
        if (kind === 'For') {
            ...
            if (!hasCycleEnd) {
                return { valid: false, error: 'После блока For должен быть блок Cycle End' };
            }
...
function main() {
    ...
    const structureCheck = validateProgramStructure(blockList);
    if (!structureCheck.valid) {
        siteLog('Ошибка: ' + structureCheck.error, true);
        return;
    }
