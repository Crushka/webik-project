...
const SNAP_TO_CORNER_OFFSET = 50;

function centerOnLast(zoom) {
    const last = getLastBlock();
    if (last) {
        const cx = parseFloat(last.style.left) || 0;
        const cy = parseFloat(last.style.top)  || 0;

        const sw = document.getElementById('aside').offsetWidth;
        const hh = document.getElementById('header').offsetHeight;

        canvasOffsetX = sw + SNAP_TO_CORNER_OFFSET - cx * zoom;
        canvasOffsetY = hh + SNAP_TO_CORNER_OFFSET - cy * zoom;
    } else {
        canvasOffsetX = 0;
        canvasOffsetY = 0;
    }
...
function getLastBlock() {
    if (droppedBlocks.length === 0) return null;
    const begin = droppedBlocks.find(b => blockKind(b) === 'Begin');
    let last = begin || droppedBlocks[droppedBlocks.length - 1];
    let cur = last;
    while (links.get(cur)?.next) cur = links.get(cur).next;
    return cur;
}
